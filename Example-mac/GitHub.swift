...
private var rootRequest: AutoCancellable?
private var reposRequest: AutoCancellable?
private var orgRequest: AutoCancellable?

private func requestRepos(org: GitHubOrganizationResponse) -> AutoCancellable {
...
public func runGitHubExample() {
    rootRequest = session.enqueue(GitHubRoot()) { response in
        do {
            switch try response.unwrap() {
            case .error(let e):
                print("Domain Error: \(e.message)")
    ...
    orgRequest = session.enqueue(GitHubOrganization(name: "SwiftScream")) { response in
        do {
            switch try response.unwrap() {
            case .error(let e):
                print("Domain Error: \(e.message)")
